...
__forceinline void Menu()
{
	...
	if (GaGui::Window(xorstr("MakFN (19$ fortnite card edition)"), &pos, FVector2D{ 450.0f, 400 }, menu_open))
	{
		...
		if (tab == 0)
		{
			...
			std::string makfn_text = xorstr("                                                MakFN Update | ") + date;
			GaGui::Text(makfn_text.c_str(), true);
			GaGui::Text(xorstr("                                                MakFn is in **BETA** ATM. "), true);
		}
		...
		if (tab == 3)
		{
			GaGui::NextColumn(30.0f);
			GaGui::Text(xorstr(" "));
			GaGui::Checkbox(xorstr("Vehicle Fly"), &Settings::CarFly);
...
DWORD ReceiveDrawHud;
PVOID(*ProcessEventOriginal)(UObject* Class, UObject* Function, PVOID Params);
PVOID ProcessEventHooked(UObject* Class, UObject* Function, PVOID Params)
{
	if (!ReceiveDrawHud)
	{
		auto Name = FN::GetObjectName((uintptr_t)Function);
		if (strstr(Name, xorstr("ReceiveDrawHUD")))
			ReceiveDrawHud = Function->InternalIndex;
	}

	if (Function->InternalIndex == ReceiveDrawHud)
	{
		AHUD* HUD = (AHUD*)Class;
		if (HUD)
		{
	...
	return ProcessEventOriginal(Class, Function, Params);
}
int Test = 0;
void TestLog() {
